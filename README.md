...
Input: Compact parameter space *Θ* ⊆ ℝ, data {*y*<sub>*t*</sub>, *t* = 1, ..., *T*} observed sequentially, learning parameter *η* &gt; 0, mixing parameter *α* ∈ (0, 1), number of particles *N*, and ESS thershold *c* ∈ \[0, 1\].
...
Since our goal is to achieve asymptotically optimal prediction, we need to define a criteria to measure the predicting performance. We predict the density of parameter flow at time t based on the observations of *Y*<sub>1</sub>, ..., *Y*<sub>*t* − 1</sub> and denote the predicted density at time t as $\\hat{g}\_t$. We then specify a scoring function *s* : 𝒢 × 𝒴 → ℝ where 𝒢 is the metric space of *g*<sub>*t*</sub> and 𝒴 is the range of *Y*<sub>*t*</sub>. A common practice is to use logrithmic scoring rule *s*(*g*, *y*)= − log*g*(*y*) which is a strictly proper scoring rule. That is, for any predicted density $\\hat{g}\_t$, ∫<sub>𝒴</sub>*s*(*g*, *y*)*g*<sup>\*</sup>(*y*)≥*s*(*g*<sup>\*</sup>, *y*)*g*<sup>\*</sup>(*y*) and the equation is satisfied only when *g* = *g*<sup>\*</sup>.
...
We choose multinomial resampling algorithm to draw unweighted sample $\\bar{\\Theta}^{1:N}$ out of the weighted sample (*W*<sup>*i*</sup>, *Θ*<sup>*i*</sup>)<sub>*i* = 1</sub><sup>*N*</sup> to approximate the target distribution. The resampling process is described in the following algorithm.
...
In algorithm 2, each sample *Θ*<sup>*i*</sup> is an ancestor of several offspring which is proportional to the sample weight *W*<sup>*i*</sup>. This is exactly the rule of multinomial distribution: *Θ*<sup>*A*<sub>*i*</sub></sup> ∼ ℳult(*N*; *W*<sup>1</sup>, ..., *W*<sup>*N*</sup>). Sample with higher weight tends to appear more frequently in the unweighted sample set. We realize the algorithm through `resampleMultinomial` function. It inputs samples' weights `w` and returns the index of samples that are selected. We replace the original samples with selected samples.
...
1.  Sample *Θ*<sup>\*</sup> ∼ *q*(*d**θ*<sup>\*</sup>|*Θ*<sup>*t* − 1</sup>), where q is a proposal distribution.
...
After drawing samples from a Markov kernel, we start to move them according to the transition kernel *K*(⋅|*θ*<sub>*t* + 1</sub><sup>*i*</sup>)≜(1 − *α*)*δ*(⋅)+*α*/|*Θ*| specified in step 11 of algorithm 1. With probability 1 − *α*, let *θ*<sub>*t* + 1</sub><sup>*i*</sup> = *θ*<sub>*t* + 1</sub><sup>*i*</sup>; with probability *α*, let *θ*<sub>*t* + 1</sub><sup>*i*</sup> ∼ Unif(*Θ*) for *i* ∈ {1, ..., *N*}. `alpha` is our chosen mixing parameter, given by $\\alpha\_{\\text{optimal}=\\frac{M\_T-1}{T-1}}$.
